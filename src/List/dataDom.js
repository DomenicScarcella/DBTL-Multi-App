import * as mp from "./art/matchphotos/mp-index.js";

const a = {title: "Cody Rhodes (c) vs. John Cena",
    pix: mp.CODY_CENA, vis: "hide-li"};
const b = {title: "Gunther (c) vs. Jey Uso",
    pix: mp.GUNTHER_JEY, vis: "hide-li"};
const c = {title: "LA Knight (c) vs. Jacob Fatu",
    pix: mp.LA_FATU, vis: "hide-li"};
const d = {title: "Bron Breakker (c) vs. Penta vs. Finn Bálor vs. Dominik Mysterio",
    pix: mp.IC_4WAY, vis: "hide-li"};
const e = {title: "Tiffany Stratton (c) vs. Charlotte Flair",
    pix: mp.TIFF_CHAR, vis: "hide-li"};
const f = {title: "Iyo Sky (c) vs. Rhea Ripley vs. Bianca Belair",
    pix: mp.IYO_RHEA_BIANCA, vis: "hide-li"};
const g = {title: "AJ Styles vs. Logan Paul",
    pix: mp.AJ_LOGAN, vis: "hide-li"};
const h = {title: "Jade Cargill vs. Naomi",
    pix: mp.JADE_NAOMI, vis: "hide-li"};
const i = {title: "Rey Mysterio vs. El Grande Americano",
    pix: mp.REY_GRANDE, vis: "hide-li"};

const Rheader = {title: "Dom's List:"};
const R1 = d;
const R2 = f;
const R3 = h;
const R4 = i;
const R5 = e;

export const listR = [Rheader, R1, R2, R3, R4, R5];
